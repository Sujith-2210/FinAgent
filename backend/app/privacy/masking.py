    ...
    def calculate_debt_intensity(self, net_worth_data: Any) -> Optional[str]:
        ...
        try:
            ...
            total_liabilities = sum(
                liability.value.units if hasattr(liability, 'value') else getattr(liability, 'value_inr', 0)
                for liability in net_worth_data.liabilities
            )
            ...
            total_liabilities = sum(
                liability.get('value', {}).get('units', 0) if isinstance(liability, dict) else 0
                for liability in getattr(net_worth_data, 'liabilities', [])
            )
