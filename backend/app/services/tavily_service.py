...
class TavilyService:
    ...
    async def search(self, query: str, max_results: int = 5, search_depth: str = "basic") -> List[Dict[str, Any]]:
        ...
        # We cache the raw results dict
        cache_key = f"tavily:{query}:{search_depth}"
        cached_results = await cache_service.cached(
            ttl_seconds=self._cache_ttl_seconds, 
            key_prefix="tavily"
        )(lambda: None)() # This is valid but cleaner to use cache_manager directly here
        
        # Better approach: Use get/set directly since we have specific logic
